...
namespace bst_check {
    ...
    template <std::totally_ordered T>
        bool validate_range_subtree(std::shared_ptr<struct node_t<T>> root, T min_val, T max_val);
    
    template <std::totally_ordered T>
        bool validate_min_subtree(std::shared_ptr<struct node_t<T>> root, T min_val);
    
    template <std::totally_ordered T>
        bool validate_max_subtree(std::shared_ptr<struct node_t<T>> root, T max_val);
};

template <std::totally_ordered T>
bool bst_check::validate_range_subtree(std::shared_ptr<struct node_t<T>> root, T min_val, T max_val) 
{
    if (!root) 
    {
        return true;
    ...
    return validate_range_subtree(root->left_child, min_val, root->value) && validate_range_subtree(root->right_child, root->value, max_val);
}

template <std::totally_ordered T>
bool bst_check::validate_min_subtree(std::shared_ptr<struct node_t<T>> root, T min_val) 
{
    if (!root) 
    {
        return true;
    ...
    return validate_range_subtree(root->left_child, min_val, root->value) && validate_min_subtree(root->right_child, root->value);
}

template <std::totally_ordered T>
bool bst_check::validate_max_subtree(std::shared_ptr<struct node_t<T>> root, T max_val) 
{
    if (!root) 
    {
        return true;
    ...
    return validate_max_subtree(root->left_child, root->value) && validate_range_subtree(root->right_child, root->value, max_val);
}
...
bool is_valid_bst(std::shared_ptr<struct node_t<T>> root) 
{
    ...
    return bst_check::validate_max_subtree(root->left_child, root->value) && bst_check::validate_min_subtree(root->right_child, root->value);
    // End TODO
}
