 ...
 * Note that the indexing of the employees is based first on the department
 ...
 * "is-valid-bst.h". You should complete both tasks.
 */
...
{
    return (lhs.department_name == rhs.department_name) && (lhs.id_in_department == rhs.id_in_department);
}

std::strong_ordering operator <=> (const struct employee_t &lhs, const struct employee_t &rhs) 
{
